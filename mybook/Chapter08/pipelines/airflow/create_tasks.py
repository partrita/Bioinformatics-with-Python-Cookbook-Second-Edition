...
s_fs_sensor = FileSensor(
    task_id="download_sensor", fs_conn_id="fs_bioinf", filepath="done.txt", dag=dag
)
...
def subsample_10p(ds, **kwargs):
    fs = FSHook("fs_bioinf")
    os.system(
        "/home/tra/anaconda3/bin/plink --recode --file {fs_path}/hapmap --noweb --out {fs_path}/hapmap10 --thin 0.1 --geno 0.1".format(
            fs_path=fs.get_path()
        )
    )
    return True
...
def subsample_1p(ds, **kwargs):
    fs = FSHook("fs_bioinf")
    os.system(
        "/home/tra/anaconda3/bin/plink --recode --file {fs_path}/hapmap --noweb --out {fs_path}/hapmap1 --thin 0.01 --geno 0.1".format(
            fs_path=fs.get_path()
        )
    )
    return True
...
def compute_pca(ds, **kwargs):
    fs = FSHook("fs_bioinf")
    os.system(
        "/home/tra/anaconda3/bin/plink --pca --file {fs_path}/hapmap1 -out {fs_path}/pca".format(
            fs_path=fs.get_path()
        )
    )
    return True
...
t_subsample_1p = PythonOperator(
    task_id="subsample_1p", provide_context=True, python_callable=subsample_1p, dag=dag
)
t_subsample_1p.set_upstream(t_uncompress_files)

t_compute_pca = PythonOperator(
    task_id="compute_pca", provide_context=True, python_callable=compute_pca, dag=dag
)
t_compute_pca.set_upstream(t_subsample_1p)

t_plot_pca = PythonOperator(
    task_id="plot_pca", provide_context=True, python_callable=plot_pca, dag=dag
)
